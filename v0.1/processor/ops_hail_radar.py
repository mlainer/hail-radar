...
TARGET_MAX_SPEED_ALLOWED = 100   # max speed to be tracked; anything faster is ignored
TARGET_MIN_SPEED_ALLOWED = 0.1   # min speed to be tracked; anything slower is ignored
IDLE_NOTICE_INTERVAL = 1.0     # time in secs waiting to, um, take action on idle (in state of "not tracking" only)
...
MIN_TRACK_TO_ACQUIRED_TIME = 0.2  # min time in secs that object needs to be tracked for it to be counted

OPS24X_INFO_QUERY_COMMAND = '??'

# OPS24x configuration parameters (sent to sensor)
OPS24X_UNITS_PREF = 'UM'            # US for MPH , UK for Km/H, UM for m/s
OPS24X_SAMPLING_FREQUENCY = 'S2'    # 10Ksps
OPS24X_TRANSMIT_POWER = 'PX'        # max power
OPS24X_MAGNITUDE_MIN = 'M>5\n'     # Magnitude must be > this
OPS24X_DECIMAL_DIGITS = 'F3'        # F-zero for no decimal reporting
OPS24X_BLANKS_PREF = 'BZ'           # Blanks pref: send 0's not silence
OPS24X_N_REPORT = 'O5'             # Number of reports
OPS24X_SPEED_REPORT = 'OS'          #Turns on Speed report
OPS24X_RANGE_REPORT = 'OD'          #Turns on Range report
OPS24X_MAG_REPORT = 'OM'             
#OPS24X_MAGFM_REPORT = 'oM'
OPS24X_UNITS_REPORT = "OU"
#OPS24X_OUTPUT_FORMAT = 'OJ' # Turns on JSON output format
#OPS24X_FFT_OUTPUT = 'OF'
...
def main_init():
    ...
    if len(sys.argv) > 1:
        ...
        serial_port.port = "/dev/ttyAMA0"  # good for linux
    serial_port.open()
    ...
    #send_ops24x_cmd("Send Number of reports : ", OPS24X_N_REPORT)
    send_ops24x_cmd("Send Enable Speed report : ", OPS24X_SPEED_REPORT)
    send_ops24x_cmd("Send Enable Range report : ", OPS24X_RANGE_REPORT)
    #send_ops24x_cmd("Send Enable Magnitude FMCW report : ", OPS24X_MAGFM_REPORT)
    send_ops24x_cmd("Send Enable Magnitude report : ", OPS24X_MAG_REPORT)
    send_ops24x_cmd("Send Enable Units report : ", OPS24X_UNITS_REPORT)
    #send_ops24x_cmd("Send Output format : ", OPS24X_OUTPUT_FORMAT)
    #send_ops24x_cmd("Send Enable FFT Output : ", OPS24X_FFT_OUTPUT)
    send_ops24x_cmd("Send Directional Preference: ", OPS24X_DIRECTION_PREF)
